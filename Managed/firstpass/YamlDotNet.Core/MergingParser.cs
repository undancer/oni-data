	...
	public sealed class MergingParser : IParser
	{
		...
		public bool MoveNext()
		{
			if (_currentIndex < 0)
			{
				...
				{
					if (!(_allEvents[num] is Scalar scalar) || !(scalar.Value == "<<"))
					{
						continue;
					}
					if (_allEvents[num + 1] is AnchorAlias anchorAlias)
					{
					...
					if (_allEvents[num + 1] is SequenceStart)
					{
						...
						{
							if (_allEvents[i] is AnchorAlias anchorAlias2)
							{
								list.Add(GetMappingEvents(anchorAlias2.Value));
							}
		...
		private IEnumerable<ParsingEvent> GetMappingEvents(string mappingAlias)
		{
			...
			int nesting = 0;
			return (from e in _allEvents.SkipWhile((ParsingEvent e) => !(e is MappingStart mappingStart) || mappingStart.Anchor != mappingAlias).Skip(1).TakeWhile((ParsingEvent e) => (nesting += e.NestingIncrease) >= 0)
