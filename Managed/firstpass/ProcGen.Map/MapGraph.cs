			...
			for (int m = 0; m < arcList.Count; m++)
			{
				for (int n = 0; n < arcList.Count; n++)
				{
					if (n != m)
					{
						...
						List<Cell> list = GetNodes(edge);
						List<Cell> list2 = GetNodes(edge2);
						if (list[0] == list2[0] && list[1] == list2[1])
						{
							Debug.LogError("Duplicate edge (site same order)");
							return;
						}
						if (list[0] == list2[1] && list[1] == list2[0])
						{
							Debug.LogError("Duplicate Edge (site differnt order)");
							return;
						}
						if (list[0].node == list2[0].node && list[1].node == list2[1].node)
						{
							Debug.LogError("Duplicate edge (site node same order)");
							return;
						}
						if (list[0].node == list2[1].node && list[1].node == list2[0].node)
						{
