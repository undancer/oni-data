	...
	public sealed class Polygon
	{
		...
		public Commonality SharesEdge(Polygon other, ref int edgeIdx, out LineSegment overlapSegment)
		{
			...
			while (num2 < vertices.Count)
			{
				...
				for (int num3 = 0; num3 < other.vertices.Count; index = num3++)
				{
					...
					Vector2 vector7 = ((num4 != 0) ? vector2 : vector);
					int num5;
					Vector2 vector8;
					...
					Vector2 vector10 = ((num5 != 0) ? vector4 : vector3);
					if (!(vector6.x < vector9.x) && (vector6.x != vector9.x || !(vector6.y < vector9.y)))
					{
						Vector2 vector11 = vector6;
						Vector2 vector12 = vector7;
						vector6 = vector9;
						vector7 = vector10;
						vector9 = vector11;
						vector10 = vector12;
					}
					if (Vector2.Distance(vector7, vector9) < 0.001f)
					{
						result = Commonality.Point;
					}
					else if (vector7.x - vector9.x > 0f || (vector7.x - vector9.x == 0f && vector7.y - vector9.y > 0f))
					{
						edgeIdx = num;
						Vector2 value;
						Vector2 value2;
						if (vector7.x - vector10.x > 0f || (vector7.x - vector10.x == 0f && vector7.y - vector9.y > 0f))
						{
							value = vector9;
							value2 = vector7;
						}
