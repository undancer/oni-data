...
public class Timelapser : KMonoBehaviour
{
	...
	private IEnumerator Render()
	{
		...
		while (true)
		{
			...
			int world_id = (previewScreenshot ? ClusterManager.Instance.GetStartWorld().id : worldsToScreenshot[0]);
			if (!freezeCamera.enabled)
			{
				...
				RefreshRenderTextureSize();
				DebugHandler.SetTimelapseMode(enabled: true, world_id);
				SetPostionAndOrtho(world_id);
				activeOverlay = OverlayScreen.Instance.mode;
				OverlayScreen.Instance.ToggleOverlay(OverlayModes.None.ID, allowSound: false);
				continue;
			}
			RenderAndPrint(world_id);
			if (!previewScreenshot)
			{
				worldsToScreenshot.Remove(world_id);
			}
