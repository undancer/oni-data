...
public static class ArtifactSequence
{
	...
	private static IEnumerator Sequence()
	{
		...
		foreach (ArtifactAnalysisStationWorkable station in Components.ArtifactAnalysisStations)
		{
			cameraTaget2 = station.gameObject;
		}
		...
		foreach (SpaceArtifact artifact in Components.SpaceArtifacts)
		{
			if (artifact != null && artifact.HasTag(GameTags.CharmedArtifact))
			{
				cameraTaget3 = artifact.gameObject;
			}
		}
		CameraController.Instance.FadeOut(1f, 2f);
		yield return new WaitForSecondsRealtime(1f);
