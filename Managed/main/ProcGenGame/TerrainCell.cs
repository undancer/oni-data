...
namespace ProcGenGame
{
	...
	public class TerrainCell
	{
		...
		public List<int> GetAvailableSpawnCellsAll()
		{
			...
			{
				list.Add(Grid.XYToCell(availableSpawnPoint.x, availableSpawnPoint.y));
			}
			return list;
		}
		...
		public List<int> GetAvailableSpawnCellsFeature()
		{
			...
			{
				list.Add(Grid.XYToCell(item.x, item.y));
			}
			return list;
		}
		...
		public List<int> GetAvailableTerrainCells()
		{
			...
			{
				list.Add(Grid.XYToCell(availableTerrainPoint.x, availableTerrainPoint.y));
			}
			return list;
		}
		...
		private void AddHighPriorityCells(HashSet<Vector2I> cells)
		{
			...
			{
				int item = Grid.XYToCell(cell.x, cell.y);
		...
		private HashSet<Vector2I> DigFeature(ProcGen.Room.Shape shape, float size, List<int> bordersWidths, SeededRandom rnd, out List<Vector2I> featureCenterPoints, out List<List<Vector2I>> featureBorders)
		{
			...
			{
				Debug.LogWarning(string.Concat("Room has no centerpoints. Terrain Cell [ shape: ", shape.ToString(), " size: ", finalSize, "] [", node.node.Id, " ", node.type, " ", node.position, "]"));
			}
		...
		private void ApplyForeground(WorldGenSettings settings, Chunk world, SetValuesFunction SetValues, float temperatureMin, float temperatureRange, SeededRandom rnd)
		{
			...
			featureSpawnPoints = DigFeature(featureSettings.shape, num, featureSettings.borders, rnd, out var featureCenterPoints, out var featureBorders);
		...
		private void ApplyBackground(WorldGen worldGen, Chunk world, SetValuesFunction SetValues, float temperatureMin, float temperatureRange, SeededRandom rnd)
		{
			...
			foreach (Vector2I availableTerrainPoint in availableTerrainPoints)
			{
				int num2 = Grid.XYToCell(availableTerrainPoint.x, availableTerrainPoint.y);
				...
				float num4 = 1f;
				Vector2 vector = new Vector2(availableTerrainPoint.x, availableTerrainPoint.y);
				if (flag3 || flag4)
				{
					float num5 = 15f;
					if (flag11)
					{
						...
						num5 = Vector2.Distance(closestEdge.First + (closestEdge.Second - closestEdge.First) * timeOnEdge, vector);
					}
				...
				if (flag8 || flag7)
				{
					...
					foreach (Edge item in edgesWithTag)
					{
						...
						num7 = Mathf.Min(Mathf.Abs(MathUtil.GetClosestPointBetweenPointAndLineSegment(segment, vector, ref closest_point)), num7);
					}
					float num8 = 7f;
					if (flag10)
					{
				...
				worldGen.GetElementForBiomePoint(world, elementBandConfiguration, availableTerrainPoint, out var element, out var pd, out var dc, num4);
		...
		private void GenerateActionCells(WorldGenSettings settings, Tag tag, HashSet<Vector2I> possiblePoints, SeededRandom rnd)
		{
			...
			{
				Vector2I item = new Vector2I((int)item2.x, (int)item2.y);
			...
			{
				int num = Grid.XYToCell(item3.x, item3.y);
				if (Grid.IsValidCell(num))
				{
		...
		{
			foreach (uint neighbor_idx in terrain_neighbors_idx)
			{
				if (allCells.Find((TerrainCell cell) => cell.site.id == neighbor_idx).node.tags.ContainsOne(noPOINeighborSpawnTagSet))
				{
					return false;
				}
			}
