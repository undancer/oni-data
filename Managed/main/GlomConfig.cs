...
public class GlomConfig : IEntityConfig
{
	...
	public GameObject CreatePrefab()
	{
		...
		GameObject gameObject = EntityTemplates.CreatePlacedEntity("Glom", text, STRINGS.CREATURES.SPECIES.GLOM.DESC, 25f, decor: DECOR.BONUS.TIER0, anim: Assets.GetAnim("glom_kanim"), initialAnim: "idle_loop", sceneLayer: Grid.SceneLayer.Creatures, width: 1, height: 1);
		Trait trait = Db.Get().CreateTrait("GlomBaseTrait", text, text, null, should_save: false, null, positive_trait: true, is_valid_starter_trait: true);
		trait.Add(new AttributeModifier(Db.Get().Amounts.HitPoints.maxAttribute.Id, 25f, text));
		...
		EntityTemplates.ExtendEntityToBasicCreature(gameObject, FactionManager.FactionID.Pest, "GlomBaseTrait", "WalkerNavGrid1x1", NavType.Floor, 32, 2f, "", 0, drownVulnerable: true, entombVulnerable: true, 293.15f, 393.15f, 273.15f, 423.15f);
